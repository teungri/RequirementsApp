...
set_include_path(XLS_SCRIPT_URL.'Classes/');
    ...
    if ($release) 
     {
      ...
      //$objPHPExcel->getActiveSheet()->getStyle(getExcelLetters($fields).($i))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
      $tmp_str="";
      $query22="select *, date_format(r_date, '%d.%m.%Y') as d1, date_format(r_released_date, '%d.%m.%Y') as d2 from releases where r_id in (".$row['r_release']."0) order by r_name asc";
    ...
    if ($glossary) 
     {
      ...
      //$objPHPExcel->getActiveSheet()->getStyle(getExcelLetters($fields).($i))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
      $tmp_str="";
      $query22="select g.* from glossary g where g.g_id in (".$row['r_glossary']."0) order by g.g_id asc";
