...
// -----------------------------------------------------------------
//
// Page: "pdf project" - converting project into pdf file 
...
if ($r_p_id=="") echo "No project selected";
else
 {
  ...
  if ($ids=="" && $srch!="") 
   {
    $query="select r.r_id from requirements r left outer join projects p on r.r_p_id=p.p_id left outer join users u on r.r_u_id=u.u_id left outer join users u2 on r.r_assigned_u_id=u2.u_id where r.r_p_id=".$r_p_id." ".stripslashes(stripslashes(stripslashes(stripslashes(stripslashes($srch)))));
  ...
  $query="select p_name from projects where p_id=".$r_p_id;
  $rs = mysql_query($query) or die(mysql_error());
  if($row=mysql_fetch_array($rs)) $pdf_name=htmlspecialchars($row['p_name']);
  ...
  $filename = PROJECT_URL."/print_project.php?r_p_id=".$r_p_id."|".$filter1."|".$filter2."|".$filter4."|".$filter5."|".$filter6."|".$filter7."|".$filter8."|".$order."|".$_SESSION['chlang']."|".$ids."|".$fields;
  
  
  ///$filename="../test4.html";  //1.7MB file
  //$filename="../test4_1.html"; //15KB file
  
  //$url = PDF_SCRIPT_URL."dompdf.php?input_file=".PROJECT_URL."/test4_1.html&save_file=1&paper=letter&orientation=".$mode."&output_file=" . rawurlencode($pdf_name.".pdf");
